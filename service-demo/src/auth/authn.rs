...
impl SignedState {
    ...
    fn decode(encoded: &str, secret: &str) -> Result<Self> {
        ...
        let signed_state: SignedState =
            serde_json::from_slice(&decoded).context("Failed to parse signed state")?;
...
impl StateCache {
    /// Create a new state cache with connection pool
    ...
    /// Create a new state cache with existing Redis pool
    pub async fn new_with_pool(pool: Pool<RedisConnectionManager>) -> Result<Self> {
        Ok(Self { pool })
    }
    ...
    pub async fn retrieve(&self, state_id: &str) -> Result<Option<AuthState>> {
        ...
        match json {
            Some(data) => {
                let state: AuthState =
                    serde_json::from_str(&data).context("Failed to parse state from Redis")?;
                Ok(Some(state))
            }
            None => Ok(None),
        }
...
impl AuthorizationUrlBuilder {
    ...
    pub async fn new(redis_url: &str) -> Result<Self> {
        Ok(Self {
            state_cache: StateCache::new(redis_url).await?,
        })
    }

    /// Create a new authorization URL builder with existing Redis pool
    pub async fn new_with_pool(redis_pool: Pool<RedisConnectionManager>) -> Result<Self> {
        Ok(Self {
            state_cache: StateCache::new_with_pool(redis_pool).await?,
        })
    }
    ...
    pub async fn build_authorize_url(&self, request: AuthorizeRequest) -> Result<String> {
        ...
        let mut auth_url = Url::parse(&dex_config.auth_url).context("Invalid Dex auth URL")?;
    ...
    pub async fn retrieve_auth_state(
        ...
        auth_state
            .validate(client_ip, client_user_agent)
